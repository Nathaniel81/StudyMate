...
function applyDarkMode() {
  ...
  // Show the content after dark mode is applied
...
function applyLightMode() {
  ...
  // Show the content after light mode is applied
  const content = document.querySelector('.content');
  if (content) {
    content.classList.remove('hide');
  }
...
if (currentMode) {
  if (currentMode === 'dark') {
    // Hide the content until dark mode is applied
    const content = document.querySelector('.content');
    if (content) {
      content.classList.add('hide');
    }

    // Apply dark mode after a short delay
